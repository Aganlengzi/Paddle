...
inline void TransCompute(const int dim, const Context& dev_ctx,
                         const DenseTensor& in, DenseTensor* out,
                         const std::vector<int>& axis) {
  switch (dim) {
    case 1:
      phi::funcs::Transpose<Context, T, 1> trans1;
    ...
    case 5:
      ...
      break;
    case 6:
      phi::funcs::Transpose<Context, T, 6> trans6;
      trans6(dev_ctx, in, out, axis);
      break;
    default:
      // for dim >= 7 situation
      phi::funcs::TransposeNormal<Context, T> trans_normal;
...
void CholeskyGradKernel(const Context& dev_ctx,
    ...
    if (upper) {
      ...
      TransCompute<Context, T>(dims.size(), dev_ctx, out_grad, &l_grad,
                                     axis);
    } else {
      l = out;
      l_grad = out_grad;
    ...
    TransCompute<Context, T>(dims.size(), dev_ctx, *x_grad, &x_grad_trans,
                                   axis);
