...
#define POTRF_INSTANCE(T, C)                                                   \
  template <>                                                                  \
  void Potrf(const phi::GPUContext& dev_ctx, \
                                   cublasFillMode_t uplo, int n, T* A,         \
                                   int lda, int* info) const {                 \
    auto handle = dev_ctx.cusolver_dn_handle();                                \
    int workspace_size = 0;                                                    \
    PADDLE_ENFORCE_GPU_SUCCESS(                                                \
        phi::dynload::cusolverDn##C##potrf_bufferSize(                    \
    ...
    PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cusolverDn##C##potrf(        \
...
#define POTRF_BATCH_INSTANCE(T, C)                                             \
  template <>                                                                  \
  void PotrfBatched(                                     \
      const phi::GPUContext& dev_ctx, cublasFillMode_t uplo,       \
      int n, T* Aarray[], int lda, int* info_array, int batch_size) const {    \
    auto handle = dev_ctx.cusolver_dn_handle();                                \
    PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cusolverDn##C##potrfBatched( \
...
void CholeskyKernel(const Context& dev_ctx,
    ...
    paddle::platform::ForRange<phi::GPUContext> for_range(dev_ctx,
                                                              tensor_size);
    if (upper) {
    ...
    memory::Copy(phi::CPUPlace(), error_info.data(), dev_ctx.GetPlace(),
